...
if (fs.existsSync(SECONDARY_MAP_FILE)) {
  fs.readFileSync(SECONDARY_MAP_FILE, 'utf8')
    .toString()
    .split('\n')
    .forEach(content => {
      const contentArr = content.split(':');
...
module.exports = {
  ...
  serializer: {
    createModuleIdFactory() {
      ...
      let nextId =
        (secondaryFileIds.length > 0
          ? secondaryFileIds[secondaryFileIds.length - 2]
          : commonFileIds[commonFileIds.length - 2]) + 1;
      ...
      return function (path) {
        ...
        if (modulePath.includes(polyfills/require.js)) {
          return ''
        }
        let moduleId =
          commonFileToIdMap[modulePath] || newsFileToIdMap[modulePath];
        console.log('anand modulePath ', modulePath, moduleId);
    ...
    processModuleFilter(modules) {
      ...
      console.log('anand modulePath 2', modulePath);
      // anand modulePath 2 node_modules/metro-config/node_modules/metro-runtime/src/polyfills/require.js
      if (typeof commonFileToIdMap[modulePath] !== 'number' || modulePath != 'node_modules/metro-config/node_modules/metro-runtime/src/polyfills/require.js') {
        return true;
      }
      return false;
    },
    ...
    postProcessBundleSourcemap: ({code, map, outFileName}) => {
      ...
      console.log('anand -- code ', code, map);
      return {code, map};
    },
  },
};
